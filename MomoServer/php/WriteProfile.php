...
try
{
	...
	if ($exist_img == 'true')
	{
		...
		{
			$query = "update momo_db.person set person_name='$name', etc='$etc', profile_url='$upload_file' where p_id='$id'";
	...
	{
		$query = "update momo_db.person set person_name='$name', etc='$etc' where p_id='$id'";
		$response = mysqli_query($con, $query);
